 ...
 *
 * Buffer: [ a b c d e f g h i j k l m n o p q r s t u v w x y z ]
 */
...
void TestHarness::output_test1() {
    ...
    ostringstream os;
    os << buf;
    QVERIFY(os.str() == "{a, b, c, d^}");

}

void TestHarness::output_test3() {
    ostringstream os;
    GapBuffer buf_move;
    for (char ch = 'a'; ch <= 'd'; ch++) {
        buf_move.insert_at_cursor(ch);
    }

    buf_move.move_cursor(-2);
    os.str("");
    os.clear();
    os << buf_move;
    QVERIFY(os.str() == "{a, b, ^c, d}");
...
void TestHarness::equal_test1() {
    GapBuffer buf1;
    GapBuffer buf2;
    for (char ch = 'a'; ch <= 'd'; ch++) {
        buf1.insert_at_cursor(ch);
        buf2.insert_at_cursor(ch);
        QVERIFY(buf1 == buf2);
        QVERIFY(!(buf1 != buf2));
    }
...
void TestHarness::equal_test2() {
    GapBuffer buf1;
    GapBuffer buf2;
    for (char ch = 'a'; ch <= 'd'; ch++) {
        buf1.insert_at_cursor(ch);
        buf2.insert_at_cursor('d' - ch);
        QVERIFY(buf1 != buf2);
        QVERIFY(!(buf1 == buf2));
    }
...
void TestHarness::compare_test1() {
    GapBuffer buf1;
    GapBuffer buf2;

    QVERIFY(buf1 == buf2);

    buf1.insert_at_cursor('a');
    buf1.insert_at_cursor('b');
    buf2.insert_at_cursor('b');
    QVERIFY(buf1 < buf2);
    QVERIFY(buf1 <= buf2);
    QVERIFY(!(buf1 > buf2));
    QVERIFY(!(buf1 >= buf2));
...
void TestHarness::compare_test2() {
    GapBuffer buf1;
    GapBuffer buf2;

    QVERIFY(buf1 == buf2);

    buf1.insert_at_cursor('c');
    buf1.insert_at_cursor('b');
    buf2.insert_at_cursor('b');
    QVERIFY(buf1 > buf2);
    QVERIFY(buf1 >= buf2);
...
void TestHarness::compare_test3() {
    GapBuffer buf1;
    GapBuffer buf2;
